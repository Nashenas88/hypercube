...
**Render/interaction flow:** `main.rs` → `app.rs`'s `HypercubeApp::view()` embeds the `Shader` widget → iced calls `HypercubeShaderProgram::update()` per event (mutates `HypercubeShaderState`) → `draw()` builds a `HypercubePrimitive` snapshot → iced calls `Primitive::prepare()` (uploads buffers via `Renderer::update_*`) then `Primitive::render()` (`Renderer::render()` draws the scene - opaque hypercube, then the shaded, depth-writing 4D-rotation-axis gizmo, then the translucent Fire/Ice/Light/Dirt batches - into an offscreen HDR target, `Renderer::composite()` blits it - plus bloom, under the Elemental theme - into iced's surface, then `render_debug_aabb()` draws debug overlays directly onto that surface, all encoding actual wgpu render passes).
