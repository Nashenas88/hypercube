...
A 4D Rubik's-cube visualizer built in Rust with `iced` 0.14 (wgpu + advanced features) and `nalgebra`. Single binary crate, no workspace.
...
- `shaders/*.wgsl` — WGSL shaders sharing `Transform4D` (`rotation_matrix`, `viewer_distance`, `sticker_scale`, `face_gap`) and `CameraUniform` structs. `math4d.wgsl` holds shared 4D math; WGSL has no import mechanism, so its contents are manually inlined into the shaders that need it (see the TODO in `shader.wgsl`).
